...
export const exerciseService = {
  ...
  async createExercise(
    ...
    const exists = (
      weightsDoc[group as keyof typeof weightsDoc] as {
        exercises: { name: string }[];
      }
    ).exercises.some((ex) => ex.name.toLowerCase() === name.toLowerCase());
    if (exists) {
      throw new Error(
  ...
  async updateExercise(
    ...
    const weightsDoc = await Weights.findOne({
      athlete: new Types.ObjectId(athleteId),
    });
    if (!weightsDoc) throw new Error("Atleta não encontrado");

    const exercise = (
      weightsDoc[group as keyof typeof weightsDoc] as any
    ).exercises.id(Id);
  ...
  async deleteExercise(athleteId: string, group: string, _id: string) {
    ...
    const groupExercises = (weightsDoc[group as keyof typeof weightsDoc] as any)
      .exercises;
